...
router.get('/get', auth.authenticateToken, (req, res, next) => {
    var query = "SELECT p.id, p.name, p.description, p.price, p.status, p.categoryId AS categoryId, c.name AS categoryName FROM product AS p INNER JOIN category AS c ON p.categoryId = c.id"
...
router.patch('/updateStatus', auth.authenticateToken, checkRole.checkRole, (req, res, next) => {
    ...
    connection.query(query, [user.status, user.id], (err, results) => {
        if (!err) {
            ...
            return res.status(200).json({
                message: "Product Status Updatedd Successfully"
            })
        } else {
            return res.status(500).json(err)
        }
